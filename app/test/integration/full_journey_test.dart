...
/// fakes: onboarding → host sign-up → event creation → QR dashboard →
/// guest join → uploads (incl. retry) → live album → favorite → settings.
void main() {
  testWidgets('full journey: host creates, guest joins & uploads, album lives',
      (tester) async {
    ...
    final uploads = FakeUploadsRepository()..failuresByFileName['flaky.jpg'] = 1;
    ...
    // ---- 1. Splash → onboarding → sign-up → home ----
    ...
    await tester.enterText(find.byType(TextFormField).at(0), 'Priya Sharma');
    await tester.enterText(
        find.byType(TextFormField).at(1), 'priya@example.com');
    ...
    await tester
        .ensureVisible(find.widgetWithText(FilledButton, 'Create event'));
    await tester.pumpAndSettle();
    ...
    final createdEvent =
        (await events.watchMyEvents().first).single;
    ...
    await tester.runAsync(() async {
      await container
          ...
          .addFiles('event-1', [makeFile('beach.jpg'), makeFile('flaky.jpg')]);
    ...
    album.addItem(FakeAlbumRepository.buildItem(
        id: 'up1', eventId: 'event-1', guestName: 'Anita'));
    album.addItem(FakeAlbumRepository.buildItem(
        id: 'up2',
        eventId: 'event-1',
        guestName: 'Anita',
